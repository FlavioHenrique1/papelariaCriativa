...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // FILTROS
    // ==============================
    const inputBusca = document.getElementById("buscarCompra");
    const selectInsumo = document.getElementById("insumoSelect");
    const tabela = document.getElementById("tabelaInsumos");
    const linhas = tabela.getElementsByTagName("tbody")[0].getElementsByTagName("tr");
    const botaoFiltrar = document.querySelector(".btn-outline-secondary");

    function filtrarTabela() {
        const texto = inputBusca.value.toLowerCase();
        const insumoSelecionado = selectInsumo.value.toLowerCase();

        for (let i = 0; i < linhas.length; i++) {
            const colunas = linhas[i].getElementsByTagName("td");

            if (colunas.length > 0) {
                const nomeInsumo = colunas[0].textContent.toLowerCase();
                const descricao = colunas[1].textContent.toLowerCase();

                const correspondeTexto =
                    nomeInsumo.includes(texto) ||
                    descricao.includes(texto);

                const correspondeSelect =
                    insumoSelecionado === "" ||
                    nomeInsumo.includes(insumoSelecionado);

                if (correspondeTexto && correspondeSelect) {
    ...
    selectInsumo.addEventListener("change", filtrarTabela);
    botaoFiltrar.addEventListener("click", filtrarTabela);
    ...
    const unidadesComConversao = ['Folhas', 'ml', 'g','pc','mm'];
